package com.cci.rest.service.Authenticate;
...
public class LoginAuthenticator {
	...
	// private static CCSLogger logger =
	// CCSLogger.getInstance(LoginAuthenticator.class);
	/**
	 * This method will authenticate user
	 * 
	 * @param userName <code>String</code>
	 * @param password <code>String</code>
	 * @return true <code>boolean</code> if user is authenticated else false.
	 */
	public AtlasUsers authenticateUser(String userName, String password) {
		...
		try {
			...
			System.out.println(srchContext);
			isUserAuthentic = true;
			
			System.out.println(userName +" is authenticate "+isUserAuthentic);
			
			SearchControls constraints = new SearchControls();
			constraints.setSearchScope(SearchControls.SUBTREE_SCOPE);
			// NOTE: The attributes mentioned in array below are the ones that will be
			// retrieved, you can add more.
			String[] attrIDs = { "distinguishedName", "sAMAccountName", "sn", "cn", "givenname", "mail",
			...
			NamingEnumeration answer = srchContext.search("DC=ccrn,DC=com", "sAMAccountName=" + userName, constraints);
			
//				For getting active users list
			String searchFilter = "(&(objectCategory=person))";
			
			//For getting userList from OU Pune
			
			NamingEnumeration userList1 = srchContext.search("OU=Pune,OU=Sites,DC=ccrn,DC=com", searchFilter,
					constraints);

			while (userList1.hasMore()) {

				Attributes attrs = ((SearchResult) userList1.next()).getAttributes();
				...
				//For getting userList from OU Facilities
				NamingEnumeration userList = srchContext.search("OU=Facilities,OU=Sites,DC=ccrn,DC=com", searchFilter,
						constraints);
...
//				End of getting active users list
		...
		user.setActiveUsersList(userListMap);
			...
			System.out.println("Users detials are printed as follow : "+user);
		return user;
